...
import icon from '../assets/icons/marketing.svg'
...
function Marketing({ data }) {
    ...
    return (
        <div className='section'>
            <span className="section_name section_1" onClick={() => setOpen(!open)}>
                ...
                <div className="on_schedule" style={{ paddingLeft: `${(startDate - 1) * 21}px` }}>
                    <div
                        ...
                        style={{ border: '1px solid #497CF6', background: '#E2EBFF', width: `${(period * 21) + 2}px` }}
                    >
                    </div>
                    <p>
                        {data.title}
            ...
            <div className={open ? "section_body show_body" : "section_body"}>
                <Banners data={data.sub[0]} parent={open} />
            </div>
        </div>
    )
}
